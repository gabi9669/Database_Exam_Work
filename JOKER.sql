...
DROP ROLE IF EXISTS [ProductManager]
GO
...
USE vizsga
GO
DROP USER IF EXISTS [Magnus] 
DROP ROLE IF EXISTS [OrderManager]
GO
...
USE vizsga
GO
DROP USER IF EXISTS [Silvia] 
DROP ROLE IF EXISTS [MarketingStat]
GO
...
IF EXISTS (SELECT name 
            ...
            WHERE name = N'FULL_BACKUP')
BEGIN
EXEC msdb.dbo.sp_delete_job @job_name = N'FULL_BACKUP', @delete_unused_schedule=1
END
GO
...
IF EXISTS (SELECT name 
            FROM msdb.dbo.sysjobs_view 
            WHERE name = N'LOG_BACKUP')
BEGIN
EXEC msdb.dbo.sp_delete_job @job_name = N'LOG_BACKUP', @delete_unused_schedule=1
END
DECLARE @jobId BINARY(16)
